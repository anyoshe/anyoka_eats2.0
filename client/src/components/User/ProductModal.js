...
const ProductModal = ({ isOpen, onClose, onSubmit, editingProduct, onProductUpdated }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (primaryImage) {
      if (typeof primaryImage === 'string') {
        ...
        const newIndex = newFiles.indexOf(primaryImage);
        if (newIndex !== -1) {
          formData.append('primaryImage', `new:${newIndex}`);
        }
      }
    }
    ...
    try {
      const url = editingProduct ? `${config.backendUrl}/api/products/${editingProduct._id}` : `${config.backendUrl}/api/products`;
      const method = editingProduct ? 'PUT' : 'POST';
      const response = await fetch(url, {
        method,
        body: formData,
  ...
  return (
    <div className={styles.modalOne}>
      <div className={styles.modalContentOne}>
        <span className={styles.closeModalOne} onClick={resetForm}>
          &times;
        </span>
        <h2 className={styles.addProductH2}>{editingProduct ? 'Edit Product' : 'Add Product'}</h2>
        <div className={styles.productImagePreviewContainer}>
          {productImages.map((image, index) => (
            <div key={index} className={styles.imagePreviewItem}>
              <img
                className={styles.productImagePreview}
                src={typeof image === 'string' ? `${config.backendUrl}${image}` : URL.createObjectURL(image)}
                alt={`Product ${index + 1}`}
              />
              <div className={styles.imageBtns}>
                <button
