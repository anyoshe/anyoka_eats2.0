          ...
          return (
            <div key={order._id} className={styles.orderCard}>
              ...
              {isCollapsed ? (
                ...
                <>
                  <p className={styles.orderInfo}>Created At: {new Date(order.createdAt).toLocaleString()}</p>
                  <p className={styles.orderInfo}>Total: KES {order.total}</p>
                  <p className={styles.orderInfo}>Payment Method: {order.paymentMethod}</p>
                  <p className={styles.orderInfo}>Payment Status: {order.paymentStatus}</p>
                  <p className={styles.orderInfo}>
                    Delivery: {order.delivery.town}, {order.delivery.location} ({order.delivery.option})
                  </p>

                  <h4 className={styles.subOrdersTitle}>Sub-Orders (Grouped by Shop):</h4>
                  <ul className={styles.subOrdersList}>
                    {order.subOrders.map((sub) => (
                      <li key={sub._id} className={styles.subOrder}>
                        <h5 className={styles.shopName}>Shop: {sub.shop?.businessName || 'Unknown Shop'}</h5>
