...
import { AuthContext } from '../../contexts/AuthContext';
import config from '../../config';
import styles from './CustomerPastOrder.module.css';
            ...
            <div key={order._id} className={styles.orderCard}>
              <div className={styles.orderHeader}>
                ...
                <div className={styles.orderSummary}>
                  <p className={styles.orderInfo}>
                    <strong>Date:</strong> {new Date(order.deliveredAt || order.updatedAt || order.createdAt).toLocaleDateString()}
                  </p>
                  <p className={styles.orderInfo}>
                    <strong>Order #:</strong> {order.orderId}
                  </p>
                  <p className={styles.orderInfo}>
                    <strong>Amount:</strong> KES {totalWithDelivery} <small>(Order: {order.total} + Delivery: {deliveryFee})</small>
                  </p>
                </div>
              ...
              {isExpanded && (
                <div className={styles.expandedSection}>
                  <p className={styles.orderDetail}><strong>Payment Method:</strong> {order.paymentMethod}</p>
                  <p className={styles.orderDetail}><strong>Payment Status:</strong> {order.paymentStatus}</p>
                  <p className={styles.orderDetail}>
                    <strong>Delivery:</strong> {order.delivery.town}, {order.delivery.location} ({order.delivery.option})
                  </p>
